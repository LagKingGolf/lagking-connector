...
class MainWindow(QMainWindow, Ui_MainWindow):
    version = 'V1.04.51'
    app_name = 'MLM2PRO-GSPro-Connector (LagKing fork)'
    # NOT a path. AppDataPaths ignores this name on Windows and macOS --
    # app_data_path() returns <working directory>/appdata regardless -- so this
    # string only names the log file and lock file inside it.
    #
    # Which also means settings live NEXT TO THE EXE, not under %LOCALAPPDATA%.
    # An upstream user who drops this build into their existing folder keeps
    # their settings automatically, so no migration is needed or possible; an
    # earlier attempt at one here could never fire and has been removed.
